	...
	<div class="row">
		<div class="col-sm-5">
			<div class="border-dashed bg-dark-2 br-10 p-4 mb-4 sticky-top">
				<div wire:loading wire:target="save, image" class="position-absolute w-100 h-100 top-0 start-0 bg-dark br-10" style="--bs-bg-opacity: 0.9; z-index: 2;">
				...
				<div class="mb-3">
					...
					<span class="text-muted fs-12">{{ url('/') }}/etiqueta/<input type="text" wire:model.defer="createArray.slug" class="border-0 bg-transparent text-muted fs-12 p-0"></span>
				...
				<button class="btn btn-success fs-14 px-4" wire:click="save" wire:loading.attr="disabled" wire:target="image, imageTable, save">Guardar Etiqueta</button>
			</div>
		</div>
                    ...
                    @foreach ($tags as $tag)
                        <tr>
                            ...
                            <td class="text-end pe-0">
                                <a class="d-inline-block px-2 py-1 rounded-2 bg-dark-4" wire:click="edit('{{ $tag -> slug }}')" data-bs-toggle="modal" data-bs-target="#editModal"><img src="{{ asset('img/admin/ico-edit.svg') }}" width="16" class="f-invert"></a>
                                <a class="d-inline-block px-2 py-1 rounded-2 bg-dark-4 ms-1" onclick="confirm('¿Seguro que deseas eliminar esta categoría?') || event.stopImmediatePropagation()" wire:click="destroy('{{ $tag -> slug }}')"><img src="{{ asset('img/admin/ico-delete.svg') }}" width="16" class="f-invert"></a>
                            </td>
                        </tr>
                    @endforeach	
					</tbody>
			...
			<div class="modal-content bg-dark-2">
				...
				<div class="modal-body">
					<div class="mb-3">
						...
						<span class="text-muted fs-12">{{ url('/') }}/categoria/<input type="text" wire:model="editArray.slug" class="border-0 bg-transparent text-muted fs-12 p-0"></span>
